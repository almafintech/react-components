...
import { DateRange, getShortMonthNames, sameDate } from "../helpers"
...
const Calendar = (props: Props) => {
  ...
  const setRangeBounds = (n: Date) => {
    ...
    else if (range[0] === undefined || (range[0] !== undefined && range[1] !== undefined) || n < range[0])
  ...
  const Cell = ({
    ...
    const isSelected = isSelectable && (range[1] ? isInRange(date, ...range) : isInRange(date, range[0], hovered))
    const isStart = isSelectable && sameDate(date, range[0], isMonth)
    const isEnd =
      isSelectable &&
      (sameDate(date, range[1], isMonth) ||
        (range[1] === undefined && isInRange(date, range[0], hovered) && sameDate(date, hovered, isMonth)))

    return (
      <div
        className={`${className} ${isToday && today} ${isLastHovered && lastHovered} ${!isSelectable && unselectable} ${
          isSelected && selected
        } ${isInRange(date, ...range) && final} ${isStart && start} ${isEnd && end}`}
  ...
  const DayCalendar = () => (
    <div className={`${calendar} ${daily} ${calendarClassName}`}>
      ...
      {getMonthPrefix(calendarMonth, calendarYear).map((name, i) => {
        ...
        return <Cell date={date} key={date.toString()} name={name.toString()} className={`${dayStyle} ${padding}`} />
      })}

      {getDaysInMonth(calendarMonth).map((name) => {
        const date = new Date(calendarYear, calendarMonth, name)
        return <Cell date={date} key={date.toString()} name={name.toString()} className={dayStyle} />
      })}
  ...
  const MonthCalendar = () => (
    <div className={`${calendar} ${monthly} ${calendarClassName}`}>
      {getShortMonthNames(locale).map((name, i) => {
        const date = range[0] && !range[1] ? new Date(calendarYear, i + 1, 0) : new Date(calendarYear, i, 1)
        return <Cell key={name} date={date} name={name.toString()} className={monthStyle} isMonth />
      })}
    </div>
  )

  return ["DAY", "DATE"].includes(variant) ? <DayCalendar /> : <MonthCalendar />
}
